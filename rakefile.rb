...
release_issue_rtm_release_steps = <<-eos
...
- [ ] if necessary, change `VERSION_SUFFIX` in [AppVeyor](#{appveyor_server_url}) to "" and initiate a build
- [ ] check build
...
- [ ] run code analysis in VS in *Release* mode and address violations (send a regular PR which must be merged before continuing)
- [ ] if necessary, change `VERSION_SUFFIX` in [AppVeyor](#{appveyor_server_url}) to appropriate "-beta123" value and initiate a build
- [ ] check build
- edit draft release in [GitHub UI](https://github.com/FakeItEasy/FakeItEasy/releases):
    - [ ] ensure completeness of release notes, including non-owner contributors, if any (move release notes forward from any pre-releases to the current release)
    - [ ] attach main nupkg and analyzer nupkg
...
- [ ] if another pre-release is planned, run `rake pre_release[new_version]` to
    - create a new draft GitHub Release
    - create a new issue (like this one) for the next release, adding it to the milestone for the next production release
- [ ] close this issue
eos
...
task :next_version, :new_version do |asm, args|
  ...
  create_release(
                 ...
                 new_version,
                 release_body,
                 release_issue_rtm_release_steps,
