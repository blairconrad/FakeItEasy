...
task :next_version, :new_version do |asm, args|
  ...
  create_release(client, repo, new_version, milestone, false)
end
...
task :pre_release, :version_suffix do |asm, args|
  ...
  create_release(client, repo, "#{version}-#{version_suffix}", milestone, true)
end
...
def create_release_issue_body(is_pre_release)
  if is_pre_release
    next_release_instruction = <<-eos.unindent
      - if there's to be a new pre-release issue
        ...
        - [ ] change `VERSION_SUFFIX` on the [CI Server][ci_server]
    eos
  else
    next_release_instruction = <<-eos.unindent
  ...
  <<-eos.unindent
    **in-progress** when all other issues on this milestone are closed.
    ...
    - [ ] if necessary, change `VERSION_SUFFIX` on the [CI Server][ci_server]
    ...
    [ci_server]: http://teamcity.codebetter.com/admin/editBuildParams.html?id=buildType:bt929
  eos
end
...
class String
  def unindent
    ...
    return self.gsub(/^ {#{left_pad_size}}/, "")
  end
end
