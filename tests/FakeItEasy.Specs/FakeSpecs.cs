namespace FakeItEasy.Specs
{
    ...
    public static class FakeSpecs
    {
        public interface IFoo
        {
            ...
            event EventHandler<string>? AnEvent;
        }
...
#pragma warning restore CS0618 // ClearConfiguration is obsolete
        ...
        [Scenario]
        public static void ResetToInitialConfigurationRemovesEventConfiguration(IFoo fake, string initialListenerString = "", string addedListenerString = "")
        {
            "Given a Fake with an event that's initially subscribed to"
                .x(() => fake = A.Fake<IFoo>(options => options.ConfigureFake(f => f.AnEvent += (s, e) => initialListenerString += e)));

            "And I add an additional listener"
                .x(() => fake.AnEvent += (s, e) => addedListenerString += e);

            "And I trigger the event"
                .x(() => fake.AnEvent += Raise.With("first payload"));

            "When I reset the configuration to its initial state"
                .x(() => Fake.ResetToInitialConfiguration(fake));

            "And I trigger the event"
                .x(() => fake.AnEvent += Raise.With(", second payload"));

            "Then the initial listener will have been fired twice"
                .x(() => initialListenerString.Should().Be("first payload, second payload"));

            "And then the additional listener will have fired once"
                .x(() => addedListenerString.Should().Be("first payload"));
        }
